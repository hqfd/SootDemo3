...
//	private static final String LoggingClassCompressName = "casper.instrument.util2.LoggingClassCompress";
//	private static final String LoggingClassCompressName = "casper.instrument.util.LoggingClassCompress";
	private static final String LoggingClassCompressName = "casper.instrument.util3.LoggingClassCompress";
//	private static final String LoggingClassCompressName = "casper.instrument.util4.LoggingClassCompress";
	...
	public void doInstrumentation(List<String> processDirs, 
		...
		try{
			...
			for(int i=0; i<processDirs.size(); i++){
			...
			for(String processDir:processDirs){
				...
				}else if(file.getName().endsWith(".clss")){
...
//				return cgen.getJavaClass().getBytes(); 
	...
	private void processMethod(ClassGen cgen, Method method, MethodInstrumentSolution solution){
		//|| (solution.getInstrumentForLogging().size() == 0
		//&& solution.getInstrumentForPushingDepth().size() == 0 && solution
		//.getInstrumentForPushingDepthAndID().size() == 0)
		...
		HashMap<BranchHandle, InstructionHandle>
		branchingMap = new HashMap<BranchHandle, InstructionHandle>();
...
//						|| insHandler.getInstruction() instanceof ATHROW){
						){
					//single dispatched object
					InstructionList insertList = new InstructionList();
					Type[] paramTypes = new Type[0];
